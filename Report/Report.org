...
pontos, definindo as suas coordenadas X e Y, e escrevendo num array do programa que corria no MCU. Mais
...
Deparamo-nos com este [[https://www.embeddedrelated.com/showarticle/113.php][artigo]], que achamos muito útil. Foi ao ler este artigo que tivemos o primeiro
contacto com o algoritmo /COBS/.

Podemos resumir o algoritmo como cada bloco que termine com o delimitador escolhido é substituído por um
bloco que começa com o tamanho do próprio bloco seguido dos dados, e o pacote final termina com o
delimitador. A sequência de operações é a seguinte:

1. Ao analisar a sequência, começamos por verificar se o primeiro valor é a /escape sequence/,
   inicializando o tamanho a escrever a /1/
2. Caso não haja mais dados a ler, escrevemos a /escape sequence/
...
5. Continuamos até encontrar a /escape sequence/, e aí escrevemos o tamanho seguido da sequência de dados
   lida diferente da /escape sequence/

Assim, a sequência 07 09 00 01 00 00 02 03 04 05 06 00 18 22 passaria a 03 07 09 02 01 01 06 02 03 04 05 06 03 18 22 00.

No caso da nossa implementação adaptada, o valor delimitador aparece no início (e não no fim do pacote,
como na implementação original do /COBS/).
...
posições das bolas e das barreiras, sendo estes enviados no final para o MCU.

Caso a bola consiga passar as barreiras, o score atualizado também é enviado para o MCU.
...
nada usar o byte inteiro na transmissão para representar a sua posição. A única mudança no MCU é a
